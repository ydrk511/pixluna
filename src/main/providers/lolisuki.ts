...
import type { CommonSourceRequest, ImageMetaData, GeneralImageData, SourceResponse } from "../../utils/type";
...
export class LolisukiSourceProvider extends SourceProvider {
  ...
  async getMetaData({ context }: { context: Context }, props: CommonSourceRequest): Promise<SourceResponse<ImageMetaData>> {
    ...
    const res = await context.http
      .post<LolisukiResponse>(LolisukiSourceProvider.RANDOM_IMAGE_URL, requestParams, {
    ...
    const url = this.config.compress ? imageData.urls.original : (imageData.urls.regular || imageData.urls.original);
